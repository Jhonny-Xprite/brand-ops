...
check_structure() {
  ...
  if [[ -f "$SQUAD_DIR/config.yaml" ]]; then
    ...
    if grep -q "entry_agent:" "$SQUAD_DIR/config.yaml" 2>/dev/null; then
      ...
      ENTRY_AGENT=$(grep "entry_agent:" "$SQUAD_DIR/config.yaml" | head -1 | sed 's/.*entry_agent:[[:space:]]*//' | tr -d '"' | tr -d "'" | tr -d '\r' | xargs)
    elif grep -qE "^[[:space:]]+- id:" "$SQUAD_DIR/config.yaml" 2>/dev/null; then
      # Has agents list, extract first agent as entry point
      ENTRY_AGENT=$(grep -E "^[[:space:]]+- id:" "$SQUAD_DIR/config.yaml" | head -1 | sed 's/.*- id:[[:space:]]*//' | tr -d '"' | tr -d "'" | tr -d '\r' | xargs)
...
check_security() {
  ...
  local api_keys=$(grep -rnE --exclude-dir=".git" --exclude-dir="node_modules" --exclude-dir="tests" --exclude-dir="test-cases" "(api[_-]?key|apikey)[[:space:]]*[:=][[:space:]]*['\"][^'\"\$\{]{8,}" "$SQUAD_DIR" 2>/dev/null | grep -vE "(\{\{|\\\$\{|process\.env|[Ee]xample|placeholder|grep|pattern|EXAMPLE|sk-1234|your-key|#.*api)" || true)
  if [[ -n "$api_keys" ]]; then
  ...
  local secrets=$(grep -rnE --exclude-dir=".git" --exclude-dir="node_modules" --exclude-dir="tests" --exclude-dir="test-cases" "(secret|password)[[:space:]]*[:=][[:space:]]*['\"][^'\"\$\{]{8,}" "$SQUAD_DIR" 2>/dev/null | grep -vE "(\{\{|\\\$\{|process\.env|[Ee]xample|placeholder|grep|pattern|EXAMPLE|secret_key|your-secret|#.*secret|#.*password|mySecret|super-secret|-secret-|-here)" || true)
  if [[ -n "$secrets" ]]; then
  ...
  local aws_access=$(grep -rnE --exclude-dir=".git" --exclude-dir="node_modules" "AKIA[A-Z0-9]{16}" "$SQUAD_DIR" 2>/dev/null | grep -vE "(EXAMPLE|grep|pattern|\.sh:|\.md:.*grep)" || true)
  ...
  # GCP Service Account
  local gcp_key=$(grep -rnE --exclude-dir=".git" --exclude-dir="node_modules" '"type"[[:space:]]*:[[:space:]]*"service_account"' "$SQUAD_DIR" 2>/dev/null || true)
  if [[ -n "$gcp_key" ]]; then
  ...
  local priv_key=$(grep -rnE --exclude-dir=".git" --exclude-dir="node_modules" "-----BEGIN.*(PRIVATE|RSA|DSA|EC).*KEY-----" "$SQUAD_DIR" 2>/dev/null || true)
  if [[ -n "$priv_key" ]]; then
  ...
  local db_urls=$(grep -rnE --exclude-dir=".git" --exclude-dir="node_modules" "(postgres|mysql|mongodb|redis)://[^:]+:[^@]+@" "$SQUAD_DIR" 2>/dev/null | grep -vE "(\{\{|\[PASSWORD\]|[Ee]xample|localhost|user:pass|:password@|:secret@|grep|pattern)" || true)
  if [[ -n "$db_urls" ]]; then
